...
describe('an @theme alias whose target is never declared (F33)', () => {
    it('RED: is reported, and the `var(…, #fff)` fallback does not excuse it', () => {
        ...
        // DERIVED from the gate's own theme list, not retyped. Upstream named
        // the three themes it has; this fork has a fourth (`sun`), and a
        // hand-written list turns "every theme reports" into "these three do" —
        // which is a weaker claim that also breaks on the next theme added.
        expect(failures[0].themes).toEqual(
            gate.THEMES.map((t: { name: string }) => t.name),
        );
...
describe('colours set in the stylesheet itself (F34, .ih-eyebrow)', () => {
    it('RED: a 9px rule painting fg-4 on the reference surface is reported', () => {
        ...
        // EVERY theme here: this synthetic sheet lets field inherit the dark
        // fg-4, and declares no `sun` block at all, so sun falls through to
        // :root's fg-4 on :root's white card. The REAL sheet restates field's
        // fg-4 brighter, so there it fails light + dark only — which is what the
        // walkthrough measured in the browser. Derived from the gate's theme
        // list rather than retyped, for the reason given on the F33 case above.
...
describe('surface precedence and the literal-hex annotation', () => {
    ...
    it('a hex annotation is the surface in EVERY theme — a fixed chrome has one colour', () => {
        // fg-1 flips with the theme (#0f172a light, #f1f5f9 dark, #fff field,
        // #000000 sun), so a fixed chrome fails in exactly the themes whose
        // foreground is dark. That asymmetry IS the point: the surface stayed
        // put while the foreground moved, which is the defect shape in the
        // photo-studio toolbar. Upstream reads `['light']` here because it has
        // no `sun`; this fork's sun theme is a WHITE ground with near-black
        // type, so it lands on the same side of the split as light.
        const src = '// contrast-surface: #0f172a\n' + 'const c = "text-[11px] text-ih-fg-1";\n';
        const r = scan(src);
        expect(r.violations).toHaveLength(1);
        expect(r.violations[0].surface).toBe('#0f172a');
        expect(r.violations[0].failures.map((f: { theme: string }) => f.theme)).toEqual([
            'light',
            'sun',
        ]);
