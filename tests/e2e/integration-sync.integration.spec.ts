...
test.describe('Core Integration API (Service Binding)', () => {
    test('PATCH /api/platform/tenants/:slug syncs tenant and admin user', async ({ request }) => {
        ...
        const res = await request.patch(`${BASE_URL}/api/platform/tenants/${slug}`, {
            headers: {
    ...
    test('PATCH /api/platform/tenants/:slug rejects requests without cf-worker header', async ({ request }) => {
