...
// Pan and Measure are gone (2026-09-08). Pan only changed the cursor — the
// canvas already scrolls when zoomed. Measure opened a calibration dialog on
// its first use, which read as a broken tool mid-inspection.
const TOOLS = [
      ...
      <div className="flex items-center gap-1 flex-wrap">
        {TOOLS.map((t) => (
          /* Literal white, not `text-ih-fg-inverse`: that token flips to
             ...
             so every unselected tool was invisible on a phone in dark mode. */
          <button
            key={t.id}
            type="button"
