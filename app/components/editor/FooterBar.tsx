...
export function FooterBar({ connected = false, status, roster = [] }: FooterBarProps) {
  ...
  return (
    <div className="hidden md:flex fixed bottom-0 inset-x-0 z-30 bg-ih-bg-card border-t border-ih-border px-4 py-1.5 items-center gap-3 text-[14px] text-ih-fg-3">
      <div className="relative">
        <Button
          variant="secondary"
          size="sm"
          onClick={() => setShortcutsOpen(!shortcutsOpen)}
          icon={<kbd className="px-1 py-0.5 bg-ih-bg-muted rounded text-[13px] font-mono border border-ih-border">?</kbd>}
        >
        ...
        {shortcutsOpen && (
          <div className="absolute bottom-full left-0 mb-2 w-[320px] bg-ih-bg-card border border-ih-border rounded-lg shadow-ih-popover z-50 p-3">
            <h4 className="text-[12px] font-bold uppercase tracking-[0.2em] text-ih-fg-3 mb-2">{m.editor_shortcuts_heading()}</h4>
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[14px]">
              {SHORTCUTS.map((s, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="flex gap-0.5">
                    {s.keys.map((k, j) => (
                      <kbd key={j} className="px-1 py-0.5 bg-ih-bg-muted rounded text-[13px] font-mono border border-ih-border min-w-[22px] text-center">{k}</kbd>
                    ))}
                  </span>
      ...
      {roster.length > 0 && (
        <div className="flex items-center gap-1 mr-2">
          {roster.slice(0, 5).map((user) => (
            <div
              key={user.userId}
              className="w-6 h-6 rounded-full bg-ih-primary text-ih-fg-inverse text-[13px] font-bold flex items-center justify-center ring-2 ring-ih-bg-card"
          ...
          {roster.length > 5 && (
            <span className="text-[13px] text-ih-fg-3 ml-1">+{roster.length - 5}</span>
          )}
        </div>
      )}
      ...
      <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded border border-ih-border font-bold text-[13px]">
