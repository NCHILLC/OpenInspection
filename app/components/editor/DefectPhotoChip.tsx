...
/**
 * Builds the per-defect "add photo" chip shared by canned and custom rows.
 *
 * A factory rather than a component because the caller renders it inline in
 * two different row types and passes the count each time; returning null when
 * the parent wires no handler keeps the chip out of the read-only surfaces.
 */
export function makeDefectPhotoChip(
    ...
    return (target: DefectPhotoTarget, count: number) =>
        onAddDefectPhoto ? (
            <Button
                variant="ghost"
                size="sm"
